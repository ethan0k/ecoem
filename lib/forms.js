...
document.write('<style type="text/css">span.styled { display: block; float: left; } input.styled { display: none; } span.styled > input { display: none; } span.styled label { display: inline-block; padding: 0 5px; } select.styled { position: relative; width: ' + selectWidth + 'px; opacity: 0; filter: alpha(opacity=0); z-index: 5; } .disabled { opacity: 0.5; filter: alpha(opacity=50); }</style>');

var Custom = {
	init: function() {
		var inputs = document.getElementsByTagName("input"), span = Array(), textnode, option, active;
		for(a = 0; a < inputs.length; a++) {
		    if((inputs[a].type == "checkbox" || inputs[a].type == "radio") && (inputs[a].className == "styled" || inputs[a].parentNode.className == "styled")) {
		...
		for(a = 0; a < inputs.length; a++) {
		    if (inputs[a].className == "styled" || inputs[a].parentNode.className == "styled") {
